...
async function loadQuestions() {
  try {
    ...
    const filteredQuestions = data.filter(q => 
      q.dificuldade?.toLowerCase() === "dificil" && q.topico?.toLowerCase() === "vetores"
    );
...
function finishGame() {
  ...
  // ✅ Salva o emblema "niveis_vetores"
  localStorage.setItem("emblema_niveis_vetores", "true");

  $questionsContainer.innerHTML = `
    <p class="final-message">
      Você acertou ${totalCorrect} de ${totalQuestions} questões!<br/>
      <span>Resultado: ${message}</span><br/>
      <strong>Você conquistou o emblema de Vetores!</strong><br/>
      <img src="/frontend/assets/emblemas/niveis_vetores.png"
           alt="Emblema Vetores"
